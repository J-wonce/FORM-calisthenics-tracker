...
function App() {
  ...
  const importHealthData = async file => {
    ...
    try {
      ...
      if (!Array.isArray(payload.measurements) || !Array.isArray(payload.workouts)) throw new Error('This is not a Big D Energy health import file.');
  ...
  if (loading) return <div className="loading">Loading Big D Energy…</div>; if (!session) return <Auth />;
  ...
  return <div className="app"><aside className="sidebar"><div className="brand"><i>B</i><span>Big D Energy<small>PERSONAL TRAINING LOG</small></span></div><nav>{nav.map(([id, label, Icon]) => <button key={id} className={tab === id ? 'active' : ''} onClick={() => setTab(id)}><Icon /><span>{label}</span></button>)}</nav><div className="privacy-badge"><ShieldCheck /><span>{status || 'Private'}<small>Cloud synchronized</small></span></div><button className="health-link" onClick={() => supabase.auth.signOut()}><LogOut /><span>Lock Big D Energy</span></button></aside><div className="content"><main>
