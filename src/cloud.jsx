...
function App() {
  ...
  const loadDay = (date, currentPlan = plan, rows = history) => { const row = rows.find(item => item.session_date === date); setDailyRowId(row?.id || null); setDailyComplete(Boolean(row?.details?.completed)); setDailyWorkout(row?.details?.workout || blankSets(currentPlan)); };
